...
4) Create tenants from python shell: 
    python3 manage.py shell
    from tenant.models import Tenant
    tenant = Tenant.objects.create(name="tenant_name", schema_name="schema", sub_domain ="sub_domain_for_schema")
    tenant.save()
5) Perform default db migration : python manage.py migrate_schemas
6) Perfrom history table migration :  python manage.py migrate_history_schemas --database=log_db
